...
void ManualFlightAssistant::mainLoop(const ros::TimerEvent& time)
{
  if (rcSbusAvailable())
  {
    ...
    const double value_map_a = 2.0
        / (sbus_bridge::SBusMsg::kMaxCmd - sbus_bridge::SBusMsg::kMinCmd);
    const double value_map_b = 1.0
        - (value_map_a * sbus_bridge::SBusMsg::kMaxCmd);

    if (abs(
        sbus_command_.channels[sbus_bridge::channel_mapping::kPitch]
    ...
    if (abs(
        sbus_command_.channels[sbus_bridge::channel_mapping::kRoll]
            - sbus_bridge::SBusMsg::kMeanCmd) > sbus_axes_zero_tolerance_)
    {
      const double normalized_command = std::max(
          std::min(
    ...
    if (abs(
        sbus_command_.channels[sbus_bridge::channel_mapping::kThrottle]
            - sbus_bridge::SBusMsg::kMeanCmd) > 3 * sbus_axes_zero_tolerance_)
    {
      const double normalized_command =
          std::max(
              std::min(
                  value_map_a
                      * sbus_command_.channels[
                          sbus_bridge::channel_mapping::kThrottle]
                      + value_map_b,
                  1.0),
              -1.0);
    ...
    if (abs(
        sbus_command_.channels[sbus_bridge::channel_mapping::kYaw]
            - sbus_bridge::SBusMsg::kMeanCmd) > sbus_axes_zero_tolerance_)
    {
      const double normalized_command = std::max(
          std::min(
  ...
  else if (joypadAvailable())
  {
    ...
    {
      velocity_command.twist.linear.x = vmax_xy_
          * joypad_command_.axes[joypad::axes::kX];
    }
    if (fabs(joypad_command_.axes[joypad::axes::kY])
        > joypad_axes_zero_tolerance_)
    {
      velocity_command.twist.linear.y = vmax_xy_
          * joypad_command_.axes[joypad::axes::kY];
    }
    if (fabs(joypad_command_.axes[joypad::axes::kZ])
        > joypad_axes_zero_tolerance_)
    {
      velocity_command.twist.linear.z = vmax_z_
          * joypad_command_.axes[joypad::axes::kZ];
    }
    if (fabs(joypad_command_.axes[joypad::axes::kYaw])
        > joypad_axes_zero_tolerance_)
    {
      velocity_command.twist.angular.z = rmax_yaw_
...
bool ManualFlightAssistant::rcSbusAvailable()
{
  ...
  else if (sbus_command_.channels[sbus_bridge::channel_mapping::kGamepadMode]
      < (sbus_bridge::SBusMsg::kMeanCmd + sbus_axes_zero_tolerance_))
  {
