...
In our high-level position controller, which is adapted from~\cite{Faessler15icra}, we compute the desired orientation $\ori{des}$, collective thrust $\vect{\thrust}{}{des}$, body rates $\vect{\bodyrates}{}{des}$, angular accelerations $\vectdot{\bodyrates}{}{des}$, and angular jerk $\vectddot{\bodyrates}{}{des}$, which are then applied by the low-level controller described in the next section.
...
This is a simple and fast procedure with the drawback that none of the desired mixer inputs $\bodytorques_{des}$ and $\thrust_{des}$ is achieved exactly if one of the rotor thrusts is clipped.
