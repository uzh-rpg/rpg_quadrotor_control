    ...
    const PloynomialTrajectorySettings& trajectory_settings)
{
  ...
  {
    ROS_ERROR("[%s] Number of way points and segments are not agreeing. "
  ...
  for (int d = 0; d < 3; d++)
  {
    ...
    Eigen::Vector3d start_conditions(start_state.velocity(d),
                                     ...
                                     start_state.jerk(d));
    Eigen::Vector3d end_conditions(end_state.velocity(d),
                                   end_state.acceleration(d),
                                   end_state.jerk(d));

    Eigen::MatrixXd coefficients_for_this_dimension;
    Eigen::VectorXd f = implementation::generateFVector(new_trajectory_settings,
                                                        way_points_d,
                                                        num_segments);
...
{
  if (!isStartAndEndStateFeasibleUnderConstraints(start_state, end_state,
                                                  max_velocity,
                                                  max_normalized_thrust,
                                                  max_roll_pitch_rate))
  {
    ROS_ERROR("[%s] Desired start or end state of trajectory is not feasible "
  ...
  else
  {
    straight_line_distance += (trajectory_settings.way_points.front()
        - start_state.position).norm();
    straight_line_distance += (end_state.position
        - trajectory_settings.way_points.back()).norm();
    for (int i = 0; i < trajectory_settings.way_points.size() - 1; i++)
    {
      straight_line_distance += (trajectory_settings.way_points[i + 1]
          - trajectory_settings.way_points[i]).norm();
    }
  }
  ...
  if ((end_state.position - start_state.position).norm() >= 0.01)
  {
    ...
    {
      init_trajectory_duration = 2
          * sqrt(straight_line_distance / acc_towards_s1_max);
    }
    else
    {
  ...
  const Eigen::VectorXd& bounds_violating_segment_times = initial_segment_times
      / initial_segment_times.sum() * init_trajectory_duration;
  ...
  PolynomialTrajectory trajectory =
      implementation::enforceMaximumVelocityAndThrust(initial_trajectory,
                                                      trajectory_settings,
                                                      max_velocity,
                                                      max_normalized_thrust,
                                                      max_roll_pitch_rate);
    ...
    const PloynomialTrajectorySettings& trajectory_settings)
{
  ...
  for (int i = 0; i < max_refinement_iterations; i++)
  {
    ...
    Eigen::VectorXd gradient = implementation::computeCostGradient(
        trajectory, trajectory_settings);
    ...
    const PloynomialTrajectorySettings& trajectory_settings)
{
  ...
  {
    ROS_ERROR("[%s] To create a ring trajectory, at least 2 way points must be "
              "specified!",
              ros::this_node::getName().c_str());
    return PolynomialTrajectory();
  }
    ...
    const double max_roll_pitch_rate)
{
  ...
  const Eigen::VectorXd& bounds_violating_segment_times = initial_segment_times
      / initial_segment_times.sum() * straight_line_distance / max_velocity;
    ...
    const PloynomialTrajectorySettings& trajectory_settings)
{
  ...
  for (int i = 0; i < max_refinement_iterations; i++)
  {
    ...
    trajectory = generateMinimumSnapRingTrajectory(segment_times,
                                                   trajectory_settings);
...
{
  Eigen::VectorXd solution = implementation::solveQuadraticProgram(
      H, f, A, b, optimization_cost);

  Eigen::MatrixXd coefficients;
  coefficients = Eigen::Map<Eigen::MatrixXd>(solution.data(),
                                             polynomial_order + 1,
                                             num_polynoms);
  ...
  for (int hh = 0; hh < std::min(poly_order, k_r + 1); hh++)
  {
    if (trajectory_settings.minimization_weights(hh) != 0.0)
    {
      ...
      for (int i = 0; i < num_terms; i++)
      {
        ...
        {
          double numerator = factorials(poly_order - i)
              / factorials(poly_order - i - hh) * factorials(poly_order - j)
              / factorials(poly_order - j - hh);
      ...
      for (int k = 0; k < num_polynoms; k++)
      {
        H_hh.block(k * (poly_order + 1), k * (poly_order + 1), num_terms,
                   num_terms) = H_times.cwiseProduct(H_basis)
            * pow(tau_dot(k), 2.0 * hh);
      }
    ...
    const Eigen::VectorXd& way_points_1D, const int num_polynoms)
{
  ...
  for (int k = 0; k < num_polynoms; k++)
  {
    ...
    {
      f_k(i) = -2.0
          * ((way_points_1D(k + 1) - way_points_1D(k)) / (poly_order + 2 - i)
    ...
    const int num_polynoms, const Eigen::VectorXd& tau_dot)
{
  ...
  const int num_constraints = 2 * num_polynoms
      + continuity_order * (num_polynoms + 1);

  Eigen::MatrixXd A = Eigen::MatrixXd::Zero(num_constraints,
                                            (poly_order + 1) * num_polynoms);
    ...
    for (int j = 0; j < num_polynoms - 1; j++)
    {
      for (int i = 0; i < poly_order - k; i++)
      {
        A(2 * num_polynoms + k * (num_polynoms - 1) + j,
          j * (poly_order + 1) + i) = factors(poly_order - k - 1 - i)
            * pow(tau_dot(j), k + 1);
      }
      A(2 * num_polynoms + k * (num_polynoms - 1) + j,
        (j + 1) * (poly_order + 1) + (poly_order - 1 - k)) = -factors(0)
          * pow(tau_dot(j + 1), k + 1);
    }
  }
  ...
  for (int k = 0; k < continuity_order; k++)
  {
    ...
    for (int i = 0; i < poly_order - k; i++)
    {
      A(2 * num_polynoms + continuity_order * (num_polynoms - 1) + k * 2 + 1,
        (num_polynoms - 1) * (poly_order + 1) + i) = factors(
          poly_order - k - 1 - i) * pow(tau_dot(num_polynoms - 1), k + 1);
    }
  }
    ...
    const Eigen::Vector3d& end_conditions)
{
  ...
  b(2 * num_polynoms - 1) = way_points_1D(num_polynoms); // trajectory end point
    ...
    const PloynomialTrajectorySettings& trajectory_settings)
{
  ...
  {
    // compute delta S
    Eigen::VectorXd delta_s = -1.0 / (num_segments - 1)
        * Eigen::VectorXd::Ones(num_segments);
    delta_s(segment) = 1.0;
    ...
    if (initial_trajectory.trajectory_type
        == polynomial_trajectories::TrajectoryType::MINIMUM_SNAP_OPTIMIZED_SEGMENTS)
    {
      trajectory = generateMinimumSnapTrajectory(segment_times,
                                                 initial_trajectory.start_state,
                                                 initial_trajectory.end_state,
                                                 trajectory_settings);
    }
    ...
    gradient(segment) = (trajectory.optimization_cost
    ...
    const Eigen::VectorXd& gradient)
{
  ...
  for (int i = 0; i < search_direction.rows(); i++)
  {
    ...
    {
      search_direction *= (max_segment_update_ratio
          * initial_trajectory.segment_times(i) / search_direction(i));
    }
  }
  ...
  search_direction = (initial_trajectory.segment_times + search_direction)
      / normalization_factor - initial_trajectory.segment_times;
    ...
    const PloynomialTrajectorySettings& trajectory_settings)
{
  ...
  const Eigen::VectorXd search_direction = computeSearchDirection(
      initial_trajectory, gradient);
  ...
  for (;;)
  {
    ...
    if (initial_trajectory.trajectory_type
        == polynomial_trajectories::TrajectoryType::MINIMUM_SNAP_OPTIMIZED_SEGMENTS)
    {
      trajectory = generateMinimumSnapTrajectory(updated_segment_times,
                                                 initial_trajectory.start_state,
                                                 initial_trajectory.end_state,
                                                 trajectory_settings);
    }
    ...
    const double max_roll_pitch_rate)
{
  if (initial_trajectory.trajectory_type
      == polynomial_trajectories::TrajectoryType::UNDEFINED)
  {
    ROS_ERROR("[%s] Cannot enforce maximum velocity/thrust on undefined "
              "trajectory",
              ros::this_node::getName().c_str());
    return initial_trajectory;
  }
      ...
      && prev_maxima.z() <= 1.01 * desired_maxima.z())
  {
    ...
    if (initial_trajectory.trajectory_type
        ...
        || initial_trajectory.trajectory_type
            == polynomial_trajectories::TrajectoryType::MINIMUM_SNAP_OPTIMIZED_SEGMENTS)
    {
      trajectory = generateMinimumSnapTrajectory(0.9 * trajectory.segment_times,
                                                 initial_trajectory.start_state,
                                                 initial_trajectory.end_state,
                                                 trajectory_settings);
    }
    ...
    const int polynomial_order)
{
  ...
  for (int segment = 0; segment < num_segments; segment++)
  {
    Eigen::MatrixXd segment_coeff = Eigen::MatrixXd::Zero(3,
                                                          polynomial_order + 1);
    ...
    const int min_poly_order)
{
  ...
  if (original_trajectory_settings.polynomial_order < min_poly_order)
  {
    ...
    ROS_WARN_THROTTLE(
        1.0, "[%s] Requested polynomial order (%d) is too small to enforce "
                                      ...
                                      double* objective_value)
{
  ...
  Eigen::MatrixXd A_lagrange = Eigen::MatrixXd::Zero(H.rows() + A_eq.rows(),
                                                     H.rows() + A_eq.rows());
